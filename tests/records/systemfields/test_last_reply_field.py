...
from helpers import add_comment, add_log_event
...
def test_last_reply_tracking_basic(example_request, user1, user2):
    ...
    comment2 = add_comment(example_request, user2.identity, "Second comment")
    example_request = Request.get_record(example_request.id)
...
def test_system_comments_excluded(example_request, user1):
    ...
    # Add user comment
    add_comment(example_request, user1.identity, "User comment")
    example_request = Request.get_record(example_request.id)
    ...
    add_log_event(example_request, system_identity, "Auto-accepted")
    example_request = Request.get_record(example_request.id)
